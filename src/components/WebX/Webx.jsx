...
<div className="relative overflow-hidden mb-6 md:hidden">
  <div ref={carouselRef} className="flex gap-4">
    {cards.map((card, idx) => (
      <div
        key={idx}
        className="relative min-w-full flex-shrink-0 rounded-2xl overflow-hidden shadow-lg h-72"
      >
        ...
        <div className="absolute inset-0 bg-gradient-to-br from-purple-700 to-black opacity-90" />

        {/* Title text */}
        <div className="relative p-6 text-white">
          <h2 className="text-lg font-semibold relative z-999">{card.title}</h2>
        </div>
      </div>
    ))}
  </div>
...
<div className="hidden md:grid md:grid-cols-3 md:gap-6 mb-12">
  {cards.map((card, idx) => (
    <div
      key={idx}
      className="relative rounded-2xl  overflow-hidden shadow-lg h-72"
    >
      {/* Background Image fills card */}
      <img
        src={card.img}
      ...
      {/* Overlay gradient for readability */}
      <div className="absolute inset-0 bg-gradient-to-br from-purple-700 to-black" />
      ...
      <div className="bg-gradient-to-br from-purple-700 to-black text-white rounded-2xl  flex flex-col md:flex-row  gap-6">
